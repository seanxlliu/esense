...
const parseAndHighlightCodeBlocks = (text: string): MessageBlock[] => {
  ...
  while ((match = codeBlockRegex.exec(text)) !== null) {
    ...
    const codeBlock = match[2]?.trim() as string;
    ...
    const highlightedCode = Prism.highlight(
      codeBlock,
      (Prism.languages[language] || Prism.languages.typescript) as Prism.Grammar,
      language,
    );
 ...
 */
export default function ChatMessage(props: { uiMessage: UiMessage, onDelete: () => void, onEdit: (text: string) => void, onRunAgain: () => void }) {
