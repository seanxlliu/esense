...
const appendMessageToHistory = (composeText: string, maxMessages = 20) => {
  if (typeof localStorage === 'undefined') return;
 ...
 */
export default function Composer({ isDeveloper, disableSend, sendMessage }: { isDeveloper: boolean; disableSend: boolean; sendMessage: (text: string) => void; }) {
  // state
      ...
      <Grid xs={12} md={3}>
        <Stack spacing={2}>
          <Box sx={{ display: 'flex', flexDirection: 'row' }}>
            ...
            <Button className='bg-indigo-500' fullWidth variant='solid' color='primary' disabled={disableSend} onClick={handleSendClicked} endDecorator={<TelegramIcon />}>
              Send
            </Button>
          </Box>
      ...
      {!!historyAnchor && (
        <Menu size='md' anchorEl={historyAnchor} open onClose={hideHistory} sx={{ minWidth: 320 }}>
          ...
          {loadMessagesFromHistory().map((text, index) => (
            <MenuItem key={'compose-history-' + index.toString()} onClick={() => pasteFromHistory(text)}>
